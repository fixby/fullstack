...
## [0.12.0.0] - 2026-03-24 — OpenClaw Skills Migration

### Changed

- **项目从 gstack 迁移到 openclaw-skills。** 所有品牌引用已更新，包括项目名称、路径和文档。
- **移除遥测功能。** 删除了所有 Supabase 相关的遥测和分析功能，包括社区健康仪表板、安装基数跟踪和崩溃聚类。
- **移除贡献者模式。** 删除了 `gstack_contributor` 配置和相关的自动报告功能。
- **更新路径引用。** 所有路径从 `~/.gstack` 更改为 `~/.openclaw`，`~/.gstack-dev` 更改为 `~/.openclaw-dev`。
- **更新二进制和脚本名称。** 所有 `gstack-*` 工具重命名为 `openclaw-*`（例如 `gstack-config` → `openclaw-config`）。
- **更新 ETHOS.md。** 移除 Garry Tan 个人哲学引用，改为通用的开发者哲学，保留 "Boil the Lake" 核心理念但移除 YC 导向内容。

### Removed

- **Supabase 集成。** 移除所有 Supabase 后端依赖。
- **遥测系统。** 移除匿名使用数据收集和分析功能。
- **社区仪表板。** 移除 `gstack-community-dashboard` 命令。
- **贡献者日志。** 移除 `~/.gstack/contributor-logs/` 目录和相关功能。

### For contributors

- 所有文档文件（ARCHITECTURE.md, CLAUDE.md, CONTRIBUTING.md, ETHOS.md）已更新以反映新的项目名称和路径。
- 安装路径从 `~/.claude/skills/gstack/` 更改为 `~/.claude/skills/openclaw-skills/`。
