/**
 * Tests for bin/openclaw-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * OPENCLAW_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'openclaw-config');
let stateDir: string;
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'openclaw-config-test-'));
});
...
describe('openclaw-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { OPENCLAW_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
