...
/** 检测 await 关键字，忽略注释。可接受的风险：字符串字面量中的 await 会触发包装（无害）。 */
function hasAwait(code: string): boolean {
  const stripped = code.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
  return /\bawait\b/.test(stripped);
}

/** 检测代码是否需要块包装器 {…} 还是表达式包装器 (…) 在异步 IIFE 内。 */
...
/** 为 page.evaluate() 包装代码，根据需要使用带块或表达式体的异步 IIFE。 */
...
export function validateReadPath(filePath: string): void {
  if (path.isAbsolute(filePath)) {
    ...
    if (!isSafe) {
      throw new Error(`绝对路径必须在以下目录内: ${SAFE_DIRECTORIES.join(', ')}`);
    }
  }
...
/**
 * 从页面提取干净的文本（去除 script/style/noscript/svg）。
 * 导出以便在元命令（diff）中复用。
 */
...
export async function handleReadCommand(
  ...
  switch (command) {
    ...
    case 'js': {
      const expr = args[0];
      if (!expr) throw new Error('用法: browse js <表达式>');
    ...
    case 'eval': {
      ...
      if (!filePath) throw new Error('用法: browse eval <js文件>');
    ...
    case 'css': {
      ...
      if (!selector || !property) throw new Error('用法: browse css <选择器> <属性>');
      const resolved = await bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'attrs': {
      const selector = args[0];
      if (!selector) throw new Error('用法: browse attrs <选择器>');
      const resolved = await bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'console': {
      ...
      if (entries.length === 0) return args[0] === '--errors' ? '(无控制台错误)' : '(无控制台消息)';
    ...
    case 'network': {
      ...
      if (networkBuffer.length === 0) return '(无网络请求)';
    ...
    case 'dialog': {
      ...
      if (dialogBuffer.length === 0) return '(无捕获的对话框)';
    ...
    case 'is': {
      ...
      if (!property || !selector) throw new Error('用法: browse is <属性> <选择器>\n属性: visible, hidden, enabled, disabled, checked, editable, focused');
      ...
      switch (property) {
        ...
        default:
          throw new Error(`未知属性: ${property}。使用: visible, hidden, enabled, disabled, checked, editable, focused`);
      }
    }
    ...
    case 'storage': {
      if (args[0] === 'set' && args[1]) {
        ...
        return `已设置 localStorage["${key}"]`;
      }
      const storage = await page.evaluate(() => ({
        localStorage: { ...localStorage },
        sessionStorage: { ...sessionStorage },
      }));
      // 隐藏看起来像密钥的值（tokens, keys, passwords, JWTs）
      ...
      for (const storeType of ['localStorage', 'sessionStorage'] as const) {
        ...
        for (const [key, value] of Object.entries(store)) {
          ...
          if (SENSITIVE_KEY.test(key) || SENSITIVE_VALUE.test(value)) {
            store[key] = `[已隐藏 — ${value.length} 个字符]`;
          }
        }
      }
    ...
    default:
      throw new Error(`未知读取命令: ${command}`);
  }
}
