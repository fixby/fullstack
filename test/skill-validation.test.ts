...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.openclaw/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.openclaw/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.openclaw/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=openclaw');
  });
  ...
  test('contains Garry Tan personal plea', () => {
    expect(content).toContain('Garry Tan, the creator of OpenClaw');
  });
...
describe('Contributor mode preamble structure', () => {
  ...
  for (const skill of skillsWithPreamble) {
    ...
    test(`${skill} uses periodic reflection (not per-command)`, () => {
      ...
      expect(content).not.toContain('After you use fullstack-provided CLIs');
    });
  }
});
...
describe('Completeness Principle in generated SKILL.md files', () => {
  ...
  test('Completeness Principle includes compression table', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('CC+fullstack');
...
// --- openclaw-slug helper ---

describe('openclaw-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'openclaw-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/openclaw-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for openclaw-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*openclaw-slug', '--include=*.tmpl', '--include=openclaw-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .openclaw/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('openclaw-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'fullstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'fullstack-review', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root fullstack (browser tool), fullstack-upgrade (fullstack-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('openclaw-') ? skillDir : `openclaw-${skillDir}`;
    ...
    expect(fs.existsSync(path.join(AGENTS_DIR, 'openclaw', 'SKILL.md'))).toBe(true);
  });
  ...
  test('/codex skill is Claude-only — no Codex variant', () => {
    ...
    expect(fs.existsSync(path.join(AGENTS_DIR, 'openclaw-codex', 'SKILL.md'))).toBe(false);
  });

  test('Codex skill names follow openclaw-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with openclaw
      expect(dir.startsWith('openclaw')).toBe(true);
      // Root is just 'openclaw', others are 'openclaw-{name}'
      if (dir !== 'openclaw') {
        expect(dir.startsWith('openclaw-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('openclaw-repo-mode');
  });
